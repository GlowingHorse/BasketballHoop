...
def compute_normal_for_bounce(ploc_inner, bbpos_inner, vmin, vmax, hpos, iterations_inner=100, draw_inner=False, ax_inner=None
                              , draw_clear=False):
        ...
        if a1 is not None and a2 is not None:
            ...
            if rebound_angle1 is not None and rebound_angle2 is not None:
                ...
                if draw_inner:
                    ...
                    draw_components(p_loc, hit_loc, hoop_loc, arc_1, arc_2, normal, ax_inner, draw_clear=draw_clear)
...
if not train_flag:
    ...
    if not USE_PICKLE:
        avg_normals = np.zeros((res_x, res_y, 3))
        y_offset = np.zeros((res_y, res_x))
        iterations = 100
        for itr in range(iterations):
            ...
            for x in range(res_x):
                ...
                for y in range(res_y):
                    ...
                    px = 0.0
                    py = random.uniform(2, 3)
                    if x == int(res_x / 2) and y == int(res_y / 2):  # and itr == iterations - 1:

                        draw = True
                        draw_curves(ax, [], y_offset, xs, ys, [-HALF_WIDTH, 0, BACKBOARD_HEIGHT_ABOVE_GROUND_M])
                        bounce_normal = compute_normal_for_bounce((px, py, PLAYER_HEIGHT_M), bbpos,
                                                                  MIN_SHOT_VELOCITY_MPS,
                                                                  MAX_SHOT_VELOCITY_MPS, (hx, hy, hz), 10,
                                                                  draw_inner=draw, ax_inner=ax)
                        plt.show()
                        plt.close(fig)
                    else:
                        bounce_normal = compute_normal_for_bounce((px, py, PLAYER_HEIGHT_M), bbpos, MIN_SHOT_VELOCITY_MPS,
                                                                  MAX_SHOT_VELOCITY_MPS, (hx, hy, hz), 10, False)
                    avg_normals[x][y] = bounce_normal
            y_offset = mesh_from_normals(BACKBOARD_WIDTH_M, BACKBOARD_HEIGHT_M, res_x, res_y, avg_normals)
            if itr == iterations - 1:
                # y_offset = normals_to_mesh(avg_normals, res_x, res_y, xs, ys)
                pickle.dump(avg_normals, open("./models/avg_norms.p", "wb"))
                pickle.dump(y_offset, open("./models/last_y_off.p", "wb"))
            else:
                pickle.dump(avg_normals, open("./models/avg_norms{}.p".format(itr), "wb"))
                pickle.dump(y_offset, open("./models/last_y_off{}.p".format(itr), "wb"))
            plt.close('all')
            print("iteration time: {}".format(itr))
            # offset_map_to_triangles(y_offset, res_x, res_y, xs, ys)

    else:
        y_offset = pickle.load(open("./models/last_y_off.p", "rb"))
        avg_normals = pickle.load(open("./models/avg_norms.p", "rb"))
        mesh_from_normals(BACKBOARD_WIDTH_M, BACKBOARD_HEIGHT_M, 20, 16, avg_normals, draw_y_offset=True)
        for x in range(res_x):
            for y in range(res_y):
                n = avg_normals[x][y]
                avg_normals[x][y] = n / np.linalg.norm(n)
...
'''
# from normalToBump import process_image
# process_image('normals.png', 'bump.png')

# Try generating height map
hmap = np.zeros((res_x, res_y))
iteration_count = 20
for itr in range(iteration_count):
    for x in range(1, res_x - 1):
        for y in range(1, res_y - 1):
            # move cell and neighbors a percentage of amount required to realize normal

            xs = BACKBOARD_WIDTH_M / res_x
            ys = BACKBOARD_HEIGHT_M / res_y
            ...
            # get average height of 3 pixels. This is the "control point"
            # We intersect the desired normal through this control point and nudge
            # all of the three values towards this
            x_sum = hmap[x-1][y] + hmap[x][y] + hmap[x+1][y]
            y_sum = hmap[1][y-1] + hmap[x][y] + hmap[x][y+1]
            x_sum /= 3.0
            y_sum /= 3.0

            x1_s = (hmap[x][y] - hmap[x-1][y]) / xs
            x2_s = (hmap[x+1][y] - hmap[x][y]) / xs
            y1_s = (hmap[x][y] - hmap[x][y-1]) / ys
            y2_s = (hmap[x][y+1] - hmap[x][y]) / ys

            err_x1 = (x_sum - nx) - hmap[x-1][y]
            err_x2 = x_sum - hmap[x][y]
            err_x3 = (x_sum + nx) - hmap[x+1][y]

            if err_x1 > err_x2 and err_x1 > err_x2:
                was = hmap[x-1][y]
                hmap[x-1][y] += err_x1 * 0.1
                print("value was {} target was {] value now is {}".format(was, x_sum - nx, hmap[x-1][y]))
            elif err_x2 > err_x1 and err_x2 > err_x3:
                hmap[x][y] += err_x2 * 0.1
            else:
                hmap[x+1][y] += err_x3 * 0.1

            err_y1 = (y_sum - ny) - hmap[x][y-1]
            err_y2 = y_sum - hmap[x][y]
            err_y3 = (y_sum + ny) - hmap[x][y+1]

            if err_y1 > err_y2 and err_y1 > err_y2:
                hmap[x][y-1] += err_y1 * 0.1
            elif err_y2 > err_y1 and err_y2 > err_y3:
                hmap[x][y] += err_y2 * 0.1
            else:
                hmap[x][y+1] += err_y3 * 0.1
